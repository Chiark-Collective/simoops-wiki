...
5. `RevisionModeService.enter` flips `_enabled$` to `true` synchronously and clears `_compareSnapshots$` — single-snapshot and compare modes are mutually exclusive.
...
10. With `_enabled$` true, live WebSocket events are dropped: `EntityService.wsBatchUpdateTokens`, `wsBatchUpdatePlants`, and `DataLoadService.forceRefreshEntities` short-circuit while `revisionMode.enabled` is true.
11. `services/filtered-entity-cache.service.ts::FilteredEntityCacheService` detects the flip via `revisionMode.enabled$` and swaps from live `EntityVisibilityService` streams to `RevisionModeService` streams (`workers$`, `plants$`, `areas$`) only after `snapshot$` emits non-null.
12. `FilteredEntityCacheService.updateCache` bypasses the plan-state filter (`entityInMode`) when `_revisionEnabled` is true, because the snapshot already represents the historical truth and live cycle/pending filters would alias distinct states.
...
17. `RevisionTimelineService.pin` sets `_pinnedIndex$`; subsequent loads trigger `RevisionModeService.enterCompare`, which flips `_enabled$` to `true`, clears `_snapshot$`, loads both snapshots in parallel (cached or via `RevisionApi.loadRevision`), pushes the pair into `_compareSnapshots$`, and sets `_loading$` to `false`.
...
20. `services/clash-state.service.ts::ClashStateService.setClashes` silently returns while `revisionMode.enabled` is true; the constructor also subscribes to `revisionMode.clashes$` when enabled and pushes them into `_clashes`.
21. `RevisionModeService.guardEdit` returns `true` when enabled, causing editing entry points to short-circuit after surfacing a standard info toast via `MessageService`.
22. Exit: `RevisionModeService.exit()` sets `_enabled$` to false, `_snapshot$` and `_compareSnapshots$` to null, and `_loading$` to false. `FilteredEntityCacheService` reverts to live streams; the caller follows with `services/data-load.service.ts::DataLoadService.forceRefreshEntities()` to re-hydrate live state and resume WebSocket processing.

## Side effects
- HTTP GET to backend per-type `/at-time` endpoints (up to 9 parallel calls per snapshot).
- `_enabled$` → `true` gates WS delta application and edit operations across the app.
...
- `RevisionModeService.guardEdit` emits a toast via `MessageService` and blocks the editing operation.
...
- Live WS event slips through while `_enabled$` is true → snapshot state could be clobbered by live data. Consumers must check `revisionMode.enabled` before applying deltas.
