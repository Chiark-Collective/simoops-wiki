...
- `services/dashboard-bootstrap-wiring.service.ts::DashboardBootstrapWiringService` — Component-scoped provider declared in `DashboardComponent` providers array. Wires one-shot bootstrap subscriptions: undo result routing, site-change side effects (presence reset, layer defaults loading), and weather initialization. Called from `DashboardComponent.ngOnInit`. `DestroyRef` is dashboard-scoped.
- `services/dashboard-interaction-wiring.service.ts::DashboardInteractionWiringService` — Component-scoped provider declared in `DashboardComponent` providers array. Wires interaction-state subscriptions (selection, building focus, panel auto-open, creation results, schedule orchestration, clash updates, etc.) and registers dashboard callback hosts with `EntityModalOrchestrator`, `EntityInteractionOrchestrator`, `ModalResultDispatchService`, and `MapEventDispatchService`. `DestroyRef` is dashboard-scoped.
...
- `services/dashboard-bootstrap-wiring.service.ts::DashboardBootstrapWiringService` / `services/dashboard-interaction-wiring.service.ts::DashboardInteractionWiringService` — Component-scoped `DestroyRef` tied to `DashboardComponent` lifetime; subscriptions clean up on component destroy.
...
- Layout persistence (`services/panel-state.service.ts::PanelStateService.persistLayoutPrefs`): persists dock position, panel widths, timeline height, lane visibility, and collapsed sections. `loadLayoutPrefs` validates fields, skips stale `timelineLaneVisibility` if stored schema version < 7, and migrates `permitsPanelWidth` → `importsPanelWidth`. `JSON.parse` failures are now surfaced via `createLogger('PanelState')` instead of being silently swallowed.
- `services/site-context.service.ts::SiteContextService.selectedShift$` and `selectedDate$` are `readonly` fields (not getters) so template async-pipe bindings get a stable Observable reference and cannot accidentally reintroduce fresh-construction-per-call.
- `services/user.service.ts::UserService.fetchCurrentUser` defers `localStorage.removeItem('simoops_pending_invite_token')` until `acceptInviteLink` resolves, preventing a network failure from stranding the user with the invite token lost and no membership applied.
